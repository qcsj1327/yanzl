...
from futures_mvp.modules.sim_trading.harness import SimExecutionResult
...
class CountingHarness:
    ...
    def execute(self, command: ExecutionCommand) -> SimExecutionResult:
        self.calls += 1
        return SimExecutionHarness().execute(command)


def _command(*, execution_target: ExecutionTarget = ExecutionTarget.MOCK) -> ExecutionCommand:
...
def _accounting_context() -> SimAccountingContext:
    return SimAccountingContext(
        account=AccountContext(
            account_id="account-1",
...
def _settlement_context_with_stale_snapshots() -> SettlementContext:
    ...
    return SettlementContext(
        ...
        calculation_key="sim-settlement",
        settled_at=NOW,
    )
...
def _context(
    *,
    ...
    return SimRunContext(
        ...
        operator_approval=operator_approval or _approval(),
...
def _coordinator(
    *,
...
) -> tuple[SimTradingCoordinator, FakePositionManager, FakeTradeRepository]:
    ...
    return (
        SimTradingCoordinator(
            harness=harness or SimExecutionHarness(policy=policy),
...
def test_safety_preflight_rejects_non_sim_without_downstream() -> None:
    ...
    assert result.status is SimRunStatus.REJECTED_NON_SIM_MODE
    assert harness.calls == 0
    assert result.raw_reports == ()


def test_safety_preflight_rejects_blocking_gates_without_harness_call() -> None:
    contexts = [
        ...
        replace(_context(), operator_approval=None),
        _context(runtime_ready=False),
        _context(unresolved_critical_incidents=("incident-1",)),
        _context(
...
def test_non_mock_command_target_rejected_before_harness() -> None:
    for target in [ExecutionTarget.PAPER, ExecutionTarget.SIM, ExecutionTarget.LIVE]:
        harness = CountingHarness()
        coordinator, _positions, _trades = _coordinator(harness=harness)

        result = coordinator.run(_context(command=_command(execution_target=target)))

        assert result.status is SimRunStatus.REJECTED_UNSUPPORTED_TARGET
        assert harness.calls == 0
        assert result.raw_reports == ()
...
def test_full_fill_e2e_reaches_trade_position_and_accounting() -> None:
    ...
    assert result.status is SimRunStatus.COMPLETED
...
def test_sim_settlement_uses_current_run_margin_and_pnl_snapshot_identity() -> None:
...
def test_reject_report_applies_oms_but_creates_no_trade_or_accounting() -> None:
    coordinator, positions, trades = _coordinator(policy=SimExecutionPolicy.IMMEDIATE_REJECT)

    result = coordinator.run(_context(accounting=_accounting_context()))

    assert result.status is SimRunStatus.APPLIED
...
def test_timeout_and_uncertain_have_no_report_or_downstream_mutation() -> None:
    for policy in [
        SimExecutionPolicy.PRE_SEND_TIMEOUT,
        SimExecutionPolicy.POST_SEND_UNCERTAIN,
    ]:
        coordinator, positions, trades = _coordinator(policy=policy)

        result = coordinator.run(_context(accounting=_accounting_context()))

        assert result.status is SimRunStatus.REJECTED_NO_REPORT
        assert result.raw_reports == ()
...
def test_duplicate_report_is_noop_and_does_not_reapply_downstream() -> None:
    ...
    assert first_result.status is SimRunStatus.COMPLETED
    assert duplicate.status is SimRunStatus.DUPLICATE
...
def test_duplicate_oms_event_noops_without_trade_or_downstream() -> None:
    ...
    coordinator = SimTradingCoordinator(
    ...
    assert result.status is SimRunStatus.DUPLICATE
...
def test_report_conflict_stops_downstream() -> None:
    ...
    assert result.status is SimRunStatus.CONFLICT
    assert result.conflict is True
    assert oms_applier.applied_events == []
    assert trades.trades == {}
    assert positions.trades == []


def test_trade_conflict_stops_position_and_accounting() -> None:
    trade_bridge = CountingTradeBridge(
        TradeBridgeResult(
...
def test_position_conflict_or_error_stops_accounting() -> None:
    for status in [PositionManagerResultStatus.CONFLICT, PositionManagerResultStatus.ERROR]:
        ...
        assert result.status in {SimRunStatus.CONFLICT, SimRunStatus.ERROR}
        assert position_manager.trades
