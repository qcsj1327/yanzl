...
- 页面入口固定为总览、配置中心、数据中心、研究、组合、纸面运行、券商只读、行情数据和系统诊断。
- 数据中心是历史行情管理中心，只负责数据、质量、同步、覆盖和管理；详见 `docs/operations/DATA_CENTER.md`。
- 配置中心展示基本配置、研究配置、纸面配置、券商配置、行情配置、安全锁、本次运行配置和配置检查。
- 配置中心只读展示或临时装配本地会话配置，不持久化配置，不写业务事实，不启用任何交易目标。
- 纸面运行时未启动时必须显示“未启动”，不得显示英文运行状态。
- 券商配置只能显示只读、影子对照和禁用状态，不得出现登录、提交、撤销按钮。
- 只读行情未配置时必须显示“未配置、不会联网、不会读取真实行情”。
- 安全锁必须显示实盘交易关闭、Paper 启用、Broker 只读、ExecutionTarget 未启用。
...
1. 历史行情链路固定为：真实数据源 -> 标准化 -> 本地库 -> 回测 / Console。
2. AkShare 只允许作为显式触发的只读数据源，不允许自动后台联网。
3. Console 同步按钮只写 `historical_bars`，不生成 command，不进入 dry-run /
   apply，不连接 Broker、CTP、SimNow。
4. Backtest 使用 `data_source=local_historical_db` 时，只读本地库；本地库无数据
   必须 `BLOCKED`，不得直接访问 AkShare。
