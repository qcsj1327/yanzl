...
- Stage L Execution Report Normalization Core 已完成后，可进入 normalized report -> OMS apply bridge planning。
- 后续 replay 仍必须保持 no OMS apply / no Accounting / no Trade generation 边界，除非另开明确 scope。
